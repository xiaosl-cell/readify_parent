...
class PromptTemplateClient:
    ...
    async def _ensure_initialized(self) -> None:
        ...
        try:
            ...
            for item in items:
                ...
                if code and system_prompt:
                    self._system_cache[code] = _convert_eval_to_langchain(system_prompt)
    ...
    async def get_system_prompt(self, code: str) -> Optional[str]:
        """
        从内存缓存中获取系统提示词，首次调用时自动从 eval 服务拉取。
        ...
        """
        await self._ensure_initialized()
        return self._system_cache.get(code)
